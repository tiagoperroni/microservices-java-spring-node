     ...
     public class ClientControllerTest {
         ...
         @Test
         public void testgetClientLogin() {

             var clientResponseLogin = new ClientResponseLogin();
             clientResponseLogin.setPassword("12345");
             clientResponseLogin.setCpf("12345");
             clientResponseLogin.setEmail("tiago@gmail.com");

             when(this.clientService.getClientLogin(anyString()))
                     .thenReturn(clientResponseLogin);

             ResponseEntity<ClientResponseLogin> client = this.clientController.getClientLogin("tiagoperroni@gmail.com");

             assertEquals("12345", client.getBody().getPassword());
             assertEquals("tiago@gmail.com", client.getBody().getEmail());

         }

         @Test
         public void testPostClientLogin() {
             ...
             when(this.clientService.clientLogin(any()))
                     .thenReturn("123654");

             var clientLogin = new ClientLogin();
             clientLogin.setEmail("tiago@gmail.com");
             clientLogin.setPassword("12323");
             ResponseEntity<String> client = this.clientController.clientLogin(clientLogin);

             assertEquals("123654", client.getBody());

         }

         @Test
         public void testRequestLoginFallBack() {

             Throwable ex = new Throwable("The password informed do not math with this e-mail.");
            ResponseEntity<Map<String, String>> error = this.clientController.requestLoginFallBack(ex);
            assertTrue(error.getBody().containsKey("Error"));
         }
